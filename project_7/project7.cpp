...
main( int argc, char *argv[ ] )
{
	...
	if( me == BOSS )
	{
		for( int dst = 0; dst < NumCpus; dst++ )
		{
			...
			MPI_Send( &BigSignal[dst*PPSize], PPSize + MAXSHIFTS, MPI_FLOAT, dst, 'B', MPI_COMM_WORLD );
		}
	}
	else
	{
		MPI_Recv( PPSignal, PPSize + MAXSHIFTS, MPI_FLOAT, BOSS, 'B', MPI_COMM_WORLD, &status );
	}
	...
	{
		MPI_Send( PPSums, MAXSHIFTS, MPI_FLOAT, BOSS, 'C', MPI_COMM_WORLD );
	}
	...
	if( me == BOSS )
	{
		...
		for( int src = 0; src < NumCpus; src++ )
		{
			...
			MPI_Recv( tmpSums, MAXSHIFTS, MPI_FLOAT, src, 'C', MPI_COMM_WORLD, MPI_STATUS_IGNORE );
			for( int s = 0; s < MAXSHIFTS; s++ )
				BigSums[s] += tmpSums[s];
		}
	}
